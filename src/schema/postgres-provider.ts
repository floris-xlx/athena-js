...
class PgCatalogClient {
  ...
  async queryColumns(schemas: string[]): Promise<ColumnQueryRow[]> {
    ...
    `
    const result = await this.pool.query<ColumnQueryRow>(sql, [schemas])
    return result.rows
  }
  ...
  async queryEnums(): Promise<Map<number, string[]>> {
    ...
    const result = await this.pool.query<EnumQueryRow>(sql)
  ...
  async queryPrimaryKeys(schemas: string[]): Promise<PrimaryKeyQueryRow[]> {
    ...
    `
    const result = await this.pool.query<PrimaryKeyQueryRow>(sql, [schemas])
    return result.rows
  }
  ...
  async queryForeignKeys(schemas: string[]): Promise<ForeignKeyQueryRow[]> {
    ...
    `
    const result = await this.pool.query<ForeignKeyQueryRow>(sql, [schemas])
    return result.rows
  }
}
